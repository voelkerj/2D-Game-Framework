...
class Entity {
 public:
  ...
  Entity(std::string name_in, Shape shape_in, State state_in, float size_x_in, float size_y_in,
         std::string sprite_path, SDL_Renderer* renderer);
  Entity(Shape shape_in, State state_in, float size_x_in, float size_y_in, std::string sprite_path,
  ...
  void apply_moment(Moment moment, Uint32 elapsed_time);
  void update_acceleration(Vector2 acceleration);
  void update_velocity(float elapsed_time);
...
Entity::Entity(std::string name_in, Shape shape_in, State state_in, float size_x_in,
               float size_y_in, std::string sprite_path, SDL_Renderer* renderer) {
  name = name_in;
  shape = shape_in;
  state = state_in;
  size_x = size_x_in;
...
Entity::Entity(Shape shape_in, State state_in, float size_x_in, float size_y_in,
               std::string sprite_path, SDL_Renderer* renderer) {
...
void Entity::load_sprite_sheet(std::string sprite_sheet_path, SDL_Renderer* renderer) {
...
void Entity::calculate_physical_properties() {
  invMass = 1 / mass;
...
void Entity::apply_force(Force force) {

  // Convert force to an acceleration
  Vector2 acceleration(force.fx / mass, force.fy / mass);

  update_acceleration(acceleration);
}

void Entity::apply_moment(Moment moment, Uint32 elapsed_time) {
  float elapsed_time_float = elapsed_time / 1000.0;  // Convert ms to s

  // Angular Velocity
  state.angular_velocity += (moment.value / MoI.value) * (M_PI / 180);

  // Angle
  state.angle += state.angular_velocity * elapsed_time_float;
}

void Entity::update_acceleration(Vector2 acceleration) {  
  state.acc_x += acceleration[0];
  state.acc_y += acceleration[1];
}

void Entity::update_velocity(float elapsed_time) {
